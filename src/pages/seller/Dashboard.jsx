...
// //     orders.forEach((order) => {
// //       let dateObj = order.createdAt ? new Date(order.createdAt) : new Date();
// //       if (isNaN(dateObj.getTime())) dateObj = new Date();

// //       const day = weekdays[dateObj.getDay()];

// //       order.products.forEach((p) => {
// //         if (!chartMap[day]) chartMap[day] = { orders: 0, revenue: 0 };
// //         chartMap[day].orders += 1;
// //         chartMap[day].revenue += (p.price || 0) * (p.quantity || 1);
// //       });
// //     });

// //     return Object.entries(chartMap).map(([day, values]) => ({
// //       day,
// //       orders: values.orders,
// //       revenue: values.revenue,
// //     }));
...
//   // Generate day-by-day chart data (fill missing dates)
//   const getSalesChartData = () => {
...
//       order.products.forEach((p) => {
//         if (!chartMap[dateStr]) chartMap[dateStr] = { orders: 0, revenue: 0 };
//         chartMap[dateStr].orders += 1;
//         chartMap[dateStr].revenue += (p.price || 0) * (p.quantity || 1);
//       });
//     });
...
//         const dateStr = d.toISOString().split("T")[0];
//         const values = chartMap[dateStr] || { orders: 0, revenue: 0 };
//         result.push({ date: dateStr, ...values });
//       }
//     }
...
//                   <YAxis />
//                   <Tooltip />
//                   <Line type="basis" dataKey="orders" stroke="#8884d8" name="Orders" />
//                   <Line type="basis" dataKey="revenue" stroke="#82ca9d" name="Revenue" />
//                 </LineChart>
//               </ResponsiveContainer>
//             </div>
...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const chartArr = Object.entries(chartMap).map(([date, values]) => ({
        date,
        orders: values.orders,
        revenue: values.revenue,
      }));
      ...
      chartArr.sort((a, b) => new Date(a.date) - new Date(b.date));
      ...
      <main className="flex-1 p-6">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Sales Chart */}
            <div className="bg-white p-6 rounded shadow mb-6">
              <h2 className="font-semibold text-xl mb-4">Sales Chart (Last 7 Days)</h2>
              <ResponsiveContainer width="100%" height={300}>
                <LineChart
                            ...
                            <tr key={productId} className="hover:bg-gray-50">
                              ...
                              <td className="border px-4 py-2">
                                <select
                                  ...
                                  disabled={updatingStatus[productId]}
                                  onChange={(e) =>
                                    handleStatusChange(order._id, p, e.target.value)
                                  }
                                  className="border p-1 rounded"
                                >
                                  <option value="pending">Pending</option>
